...
$con_name = getAnyNameById($con, $con_id);
$this_effect_val = 0;
$terms_values = array();
$last_result = 0;
...
if(isset($_GET['service_id'])  && $_GET['service_id'] != ''){
	...
	foreach($terms as $term){
		...
		if($calc_type == 'moyen'){
			$total_term = $term->get_mat_term_avg($cur_service, 'student', $std);
			$grad = '';
		} elseif($calc_type == 'points'){
			$total_term = $term->get_mat_term_points($cur_service, 'student', $std);
		} elseif($calc_type == 'per'){
			$total_term = $term->get_mat_term_per($cur_service, 'student', $std);
		} elseif($calc_type == 'skills'){
			$total_term = $term->get_mat_term_grad($cur_service, 'student', $std);
		} 
		$terms_values[$i-1]= array($term->name, $total_term);
...
if(count($terms_values) > 0){
	...
	foreach($terms_values as $key=> $array){
		...
		$myPicture->writeLabel(array("Average"),$key, $LabelSettings);
	}
}
