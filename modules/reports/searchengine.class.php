...
class SearchEngine{
	...
	public function loadQuery($id){
		...
		$show_ser = $proc->serial == '1' ? true : false;
		$signature = $proc->signature == '1' ? true : false;
		return write_html('div', 'class="ui-corner-all ui-state-highlight" style="padding:5px"',
			write_html('h2', '', $proc->name)
		).
		$this->loadQueryResult($sql, $selfields, $main_order, $grouped, $show_ser, $extras, $proc->template, $signature);
	}

	public function loadQueryResult($sql, $selfields, $main_order, $grouped=0, $show_ser, $extras, $template='', $signature=false){
		global $this_system, $lang;
