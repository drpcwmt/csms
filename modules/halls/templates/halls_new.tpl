<form id="new_resource_form">
  [@toolbox]
  <div class="ui-corner-all ui-state-highlight">
      <input type="hidden" value="" name="id" >
      <table border="0" cellspacing="0" width="100%">
        <tbody>
          <tr>
            <td valign="middel" width="120"><label class="label ui-widget-header ui-corner-left">[#hall]</label></td>
            <td><input class="ui-state-default ui-corner-right required" id="hall_name" name="name" value="" type="text"></td>
          </tr>
          <tr>
            <td valign="middel" width="120"><label class="label ui-widget-header ui-corner-left">[#room]</label></td>
            <td><input class="ui-state-default ui-corner-right" id="room_no" name="room_no" value="" type="text"></td>
          </tr>
          <tr>
            <td valign="middel" width="120"><label class="label ui-widget-header ui-corner-left">[#max_size]</label></td>
            <td><input class="ui-state-default ui-corner-right" id="max_size" name="max_size" value="" type="text"></td>
          </tr>
        </tbody>
      </table>
   </div>
</form>
