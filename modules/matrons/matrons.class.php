...
class Matrons extends Employers {
    ...
    public function loadLayout() {
      ...
      $layout->template = 'modules/matrons/templates/matrons_layout.tpl';
    ...
    static function getList() {
		global $busms;
      $sql = do_query_array("SELECT * FROM matrons ORDER BY emp_id", $busms->database, $busms->ip);
      $out = array();
      foreach($sql as $row){
        $out[] = new Matrons($row->id, $busms);
      }
      return $out;
    }
    ...
    static function loadMainLayout() {
      $layout = new Layout();
      $layout->template = 'modules/matrons/templates/matrons_main.tpl';
      ...
      foreach($matrons as $matron) {
        $d = new Layout($matron);
        $d->template = 'modules/matrons/templates/matron_list_item.tpl';
    ...
    static function addMatron($post) {
      ...
      if( do_insert_obj(array('id'=> $matron_id), 'matrons',  $busms->database, $busms->ip)!==false){
        $answer['id'] = $matron_id;
        $answer['error'] = "";
      } else {
        global $lang;
    ...
    static function delMatron($post){
    	global $busms;
      $matron_id = $post['id'];
      if( do_delete_obj("id=$matron_id", 'matrons' ,  $busms->database, $busms->ip)!==false){
        $answer['id'] = $matron_id;
        $answer['error'] = "";
      } else {
        global $lang;
	...
	static function importForm(){
		$layout = new Layout();
		$layout->template = 'modules/matrons/templates/import_form.tpl';
		$hrmss = HrMS::getList();
		$layout->con='matrons';
	...
	static function importMatrons($hrms_id){
		...
		if(count($emps) > 0){
			foreach($emps as $emp){
				...
				$chk = do_query_obj("SELECT id FROM matrons WHERE emp_id=".$emp->id." AND hrms_id=$hrms_id", $this_system->database, $this_system->ip);
				if( $chk  == false){
					if(do_insert_obj(array('emp_id'=>$emp_id, 'hrms_id'=> $hrms_id, 'name_ltr'=>$emp_ltr, 'name_rtl'=>$emp_rtl), "matrons",  $this_system->database, $this_system->ip)=== false){
						$error = $lang['error_updating'];
					} else {
						$num++;
					}
				}  else {
					if(do_update_obj(array('name_ltr'=>$emp_ltr, 'name_rtl'=>$emp_rtl), "emp_id=$emp_id AND hrms_id= $hrms_id", "matrons",  $this_system->database, $this_system->ip)=== false){
						$error = $lang['error_updating'];
					} 
				}
			}
