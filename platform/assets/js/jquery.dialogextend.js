(function() {
  ...
  $.widget("ui.dialogExtend", {
    ...
    _initButtons: function() {
      ...
      buttonPane.append('<a class="ui-dialog-titlebar-restore ui-state-default" href="#"><span class="ui-icon ' + this.options.icons.restore + '" title="restore">restore</span></a>').find('.ui-dialog-titlebar-restore').attr("role", "button").mouseover(function() {
	...
	_recallibrateCorners: function($titleBar){ // Special function 
		var $titleBar = $(this.element[0]).dialog("widget").find(".ui-dialog-titlebar");
		var $buttonPane = $titleBar.find('.ui-dialog-titlebar-buttonpane');
		_results = [];
		  for (name in this.modes) {
			if (this["_verifyOptions_" + name]) {
			  _results.push(this["_verifyOptions_" + name]());
		...
		if(_results.length>0){
			$buttonPane.find(".ui-dialog-titlebar-close").addClass('ui-state-default ui-corner-right').css('padding', '0');
			$buttonPane.find('a').removeClass('ui-corner-all');
			$buttonPane.find('a').last().addClass('ui-corner-left');
		} else {
			$buttonPane.find(".ui-dialog-titlebar-close").addClass('ui-state-default ui-corner-all');
		}
	},
