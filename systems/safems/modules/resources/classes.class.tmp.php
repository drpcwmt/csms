...
class Classes{
		...
		if(!isset($this->services)){
			...
			//	print_r($services);
				foreach($services as $service ){
					$ser = new services($service->services, $this->sms);
					if(isset($ser->id)){
						$out[] = $ser;
					}
				}
	...
	public function getStudentsIds(){ // DEpricated
		if(!isset($this->studentsIds)){
			...
			$sql .= " ORDER BY $db_student.student_data.status, $db_student.student_data.sex, $db_student.student_data.$field_order_name";
		//	$sql .= " ORDER BY $db_student.student_data.status,$db_student.student_data.$field_order_name";
			...
			if(count($students) > 0){
				...
				//	if($student->join_date < $this->sms->getYearSetting('end_date') || $student->join_date == ''){;
					if($student->status==1){
						...
						if($all_stat===true){
							$std_arr[] =  $student;
						} elseif(is_array($all_stat)&& in_array($student->status, $all_stat)){
							$std_arr[] =  $student;
						} 
					}
				}
	...
	public function loadFeesLayout(){
		...
		$layout->class_name = $this->getName();
		$students = $this->getStudents(array(1,3));
